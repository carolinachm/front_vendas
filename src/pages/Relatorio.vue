<template>
  <div class="container">
    ...
    <v-form ref="form" v-model="valid" lazy-validation>
      ...
      <div>
        <v-checkbox v-model="checkbox" label="Produto X Vendas" required></v-checkbox>
      </div>
  
      <v-checkbox v-model="checkbox1" label="Item mais pesquisados" required></v-checkbox>
      <v-checkbox v-model="checkbox2" label="Venda x Perfil" required></v-checkbox>
      ...
      <v-layout row wrap>
        <v-flex xs12 sm6 md4>
          <v-menu ref="menu" :close-on-content-click="false" v-model="menu" :nudge-right="40" :return-value.sync="date" lazy transition="scale-transition" offset-y full-width min-width="290px">
            <v-text-field slot="activator" v-model="date" label="Data inicial" prepend-icon="event" readonly></v-text-field>
        ...
        <v-flex xs12 sm6 md4>
          <v-dialog ref="dialog" v-model="modal" :return-value.sync="date" persistent lazy full-width width="290px">
            <v-text-field slot="activator" v-model="date" label="Data final" prepend-icon="event" readonly></v-text-field>
      ...
      <router-link to="/RelatorioGerado"><div class="bt">GerarRelatorio</div></router-link>
